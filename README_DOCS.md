...
The `docs/` directory is the central Knowledge Base and User Interface for the Proompts repository. It contains the Markdown files that build the static MkDocs documentation site, including essential guides, usage instructions, and a complete catalog of all prompts.
...
2. **Generated Artifacts (Do NOT edit manually, Do NOT commit)**
   These files are automatically generated by the scripts in `engine/scripts/` as build artifacts. Any manual changes made to them will be overwritten during the next CI/CD build or local script execution, and they should not be committed to git.
   *Examples:* `docs/index.md`, category pages like `docs/clinical.md`, and prompt/workflow pages under `docs/prompts/` and `docs/workflows/`.
