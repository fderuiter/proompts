...
1. Create a JSON file that contains the prompt text.
...
1. When you make a new directory, also add an `overview.json` file inside it that briefly explains the prompts in that folder.
...
1. Verify the JSON formatting by running `./scripts/validate_json.sh`.
...
Make sure the `jq` command-line tool is available. If it's missing, install it using your package manager.
...
- **`generate-overviews.yml`** automatically creates an `overview.json` file in any new prompt directory and updates the docs index.
- **`repo-checks.yml`** verifies file naming conventions, ensures all files use the `.json` extension, and checks that each directory contains `overview.json`.
