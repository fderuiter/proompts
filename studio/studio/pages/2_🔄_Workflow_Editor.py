...
@st.fragment
def render_workflow_steps_and_visualizer():
    def add_branch(idx):
        st.session_state['wf_steps'][idx]['next'].append({"target": ""})
        st.toast("Branch added")
        
    def remove_step(idx):
        st.session_state['wf_steps'].pop(idx)
        st.toast("Step removed")
    ...
    for i, step in enumerate(st.session_state['wf_steps']):
        with st.expander(f"Step: {step.get('step_id', 'Un-named')}"):
            ...
            st.button("Add Branch", key=f"add_branch_{i}", on_click=add_branch, args=(i,))
                
            st.button("Remove Step", key=f"del_step_{i}", on_click=remove_step, args=(i,))
...
render_workflow_steps_and_visualizer()
