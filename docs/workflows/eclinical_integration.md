...
```mermaid
graph TD
    INPUT_integration_description([Input: integration_description])
    INPUT_mapping_requirements([Input: mapping_requirements])
    INPUT_validation_info([Input: validation_info])
    integration_blueprint[integration_blueprint<br><i>01_architect_integration_blueprint.prompt.md</i>]
    INPUT_integration_description -. input .-> integration_blueprint
    integration_blueprint -->|sequential| mapping_playbook
    mapping_playbook[mapping_playbook<br><i>02_data_mapping_transformation_playbook.prompt.md</i>]
    INPUT_mapping_requirements -. input .-> mapping_playbook
    mapping_playbook -->|sequential| validation_checklist
    validation_checklist[validation_checklist<br><i>03_regulatory_validation_checklist.prompt.md</i>]
