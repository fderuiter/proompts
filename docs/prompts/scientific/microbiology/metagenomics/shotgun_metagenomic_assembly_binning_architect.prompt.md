---
title: shotgun_metagenomic_assembly_binning_architect
---

# shotgun_metagenomic_assembly_binning_architect
...
[View Source YAML](https://github.com/fderuiter/proompts/blob/main/prompts/scientific/microbiology/metagenomics/shotgun_metagenomic_assembly_binning_architect.prompt.yaml)

```yaml
---
name: shotgun_metagenomic_assembly_binning_architect
version: 1.0.0
description: Architects robust and mathematically rigorous pipelines for the assembly and binning of short-read and long-read shotgun metagenomic data to recover metagenome-assembled genomes (MAGs).
authors:
...
variables:
  - name: sequencing_technology
    type: string
    description: The sequencing platform utilized (e.g., Illumina paired-end, PacBio HiFi, Oxford Nanopore) dictating error profiles and read lengths.
  - name: environmental_context
    type: string
    description: The source of the microbiome sample (e.g., human gut, marine sediment, deep-sea hydrothermal vent), which influences microbial diversity and strain heterogeneity.
  - name: assembly_graph_algorithm
    type: string
    description: The core algorithm utilized for resolving the metagenomic assembly graph (e.g., de Bruijn graphs, Overlap-Layout-Consensus).
model: gpt-4o
modelParameters:
  temperature: 0.1
  maxTokens: 4096
...
testData:
  - sequencing_technology: "Illumina NovaSeq 6000 2x150bp paired-end combined with Oxford Nanopore R10.4 long reads"
    environmental_context: "Human gut microbiome with high intra-species strain heterogeneity"
    assembly_graph_algorithm: "Hybrid de Bruijn graph and Overlap-Layout-Consensus (OLC) resolution"
evaluators:
  - type: regex
    pattern: "(?i)\\\\[a-zA-Z]+"
