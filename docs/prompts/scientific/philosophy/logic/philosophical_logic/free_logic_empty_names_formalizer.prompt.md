---
title: Free Logic Empty Names Formalizer
---

# Free Logic Empty Names Formalizer
...
[View Source YAML](https://github.com/fderuiter/proompts/blob/main/prompts/scientific/philosophy/logic/philosophical_logic/free_logic_empty_names_formalizer.prompt.yaml)

```yaml
---
name: Free Logic Empty Names Formalizer
version: 1.0.0
description: A highly rigorous prompt designed to systematically formalize and evaluate propositions containing non-denoting terms or empty names using designated Free Logic semantics (Positive, Negative, or Neutral) to prevent ontological inflation and deductive explosion.
authors:
...
variables:
  - name: PROPOSITION
    description: The natural language proposition containing at least one potentially empty name or non-denoting term (e.g., "Pegasus is a flying horse").
    required: true
  - name: FREE_LOGIC_SYSTEM
    description: The specific Free Logic semantics to apply (e.g., Positive Free Logic, Negative Free Logic, Neutral Free Logic, or Supervaluationist Free Logic).
    required: true
  - name: ONTOLOGICAL_DOMAIN
    description: The specified domain of quantification, explicitly distinguishing between the inner domain of existent objects and the outer domain of non-existent objects (if applicable).
    required: true
model: gpt-4o
modelParameters:
  temperature: 0.1
...
testData:
  - inputs:
      PROPOSITION: "Pegasus is a flying horse."
      FREE_LOGIC_SYSTEM: "Negative Free Logic"
      ONTOLOGICAL_DOMAIN: "Strict Actualist Domain (Empty Outer Domain)"
    expected: "Syntactic Formalization"
  - inputs:
      PROPOSITION: "Sherlock Holmes is a detective, therefore someone is a detective."
      FREE_LOGIC_SYSTEM: "Positive Free Logic (Dual-Domain Semantics)"
      ONTOLOGICAL_DOMAIN: "Inner Domain of actual entities, Outer Domain of fictional entities"
    expected: "Existential Import & Quantificational Analysis"
evaluators:
  - string:
      regex: '(?i)(syntactic formalization|semantic evaluation|existential import)'
