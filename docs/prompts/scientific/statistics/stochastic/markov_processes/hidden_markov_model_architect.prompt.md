---
title: hidden_markov_model_architect
---

# hidden_markov_model_architect
...
[View Source YAML](https://github.com/fderuiter/proompts/blob/main/prompts/scientific/statistics/stochastic/markov_processes/hidden_markov_model_architect.prompt.yaml)

```yaml
---
name: "hidden_markov_model_architect"
version: "1.0.0"
description: "Acts as a Principal Statistician to derive maximum likelihood estimation architectures for continuous-density Hidden Markov Models using the Baum-Welch algorithm and EM methodology."
authors:
...
variables:
  - name: "observation_sequence"
    description: "The sequence of observed data vectors."
    required: true
  - name: "state_space_dimensionality"
    description: "The number of hidden states in the Markov chain."
    required: true
  - name: "emission_distribution_type"
    description: "The parametric family of the state-conditional emission probability distributions."
    required: true
model: "gpt-4o"
modelParameters:
  temperature: 0.1
...
testData:
  - inputs:
      observation_sequence: 'A time series of $T=1000$ daily financial asset returns $y_t \in \mathbb{R}$.'
      state_space_dimensionality: '$N = 3$ corresponding to distinct volatility regimes.'
      emission_distribution_type: 'Gaussian distribution parameterized by state-dependent mean $\mu_i$ and variance $\sigma_i^2$.'
    expected: "Baum-Welch"
evaluators:
  - type: "regex_match"
    pattern: "(?i)Expectation|Maximization|forward|backward|gamma"
