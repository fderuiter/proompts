---
title: social_isolation_epidemiological_contagion_modeler
---

# social_isolation_epidemiological_contagion_modeler
...
[View Source YAML](https://github.com/fderuiter/proompts/blob/main/prompts/scientific/psychology/epidemiology/global_mental_health/social_isolation_epidemiological_contagion_modeler.prompt.yaml)

```yaml
---
name: social_isolation_epidemiological_contagion_modeler
version: 1.0.0
description: A highly robust, expert-level prompt designed to mathematically model and systematically map the computational spread of social isolation and chronic loneliness as a behavioral contagion across massive population networks utilizing multi-modal epidemiological data proxies.
authors:
  - Population Behavioral Sciences Genesis Architect
metadata:
  domain: scientific
  sub_domain: psychology/epidemiology/global_mental_health
  complexity: high
  frameworks:
    - Epidemiological Contagion Models
    - Network Percolation Theory
    - Big Data Behavioral Proxies
    - Mass Behavior Dynamics
variables:
  - name: multimodal_data_schema
    description: Strict JSON/CSV schema definition detailing ingestion parameters for massive-scale demographic, social connectivity telemetry, and psychometric profiles for >50M agents.
  - name: isolation_transmission_parameters
    description: Parameters defining the behavioral contagion dynamics of chronic loneliness, including systemic risk amplification, urban isolation density, and baseline network atrophy.
  - name: structural_intervention_objective
    description: The targeted macro-level behavioral stabilization goal, such as modeling thresholds to arrest isolation contagion using localized community seeding and architectural integration.
model: gpt-4o
modelParameters:
  temperature: 0.1
  maxTokens: 8192
...
testData:
  - multimodal_data_schema: |
      {
        "agents": [{"agent_id": "string", "baseline_loneliness_index": "float", "social_network_degree": "integer", "urban_density_cluster": "float"}],
        "connectivity_telemetry": [{"agent_id": "string", "timestamp_ms": "integer", "interaction_frequency": "float", "isolation_amplification_score": "float"}]
      }
    isolation_transmission_parameters: |
      {
        "network_atrophy_rate": 1.35,
        "urban_isolation_density": 0.78,
        "recommender_system_alienation_weight": 0.88
      }
    structural_intervention_objective: Simulate the arrest of the social isolation cascade by deploying localized community engagement nodes on high-centrality intersections and injecting digital cognitive behavioral interventions when loneliness index exceeds the 85th percentile.
evaluators:
  ...
  - name: strictly_authoritative_persona_adherence
    model:
      prompt: "Assess the generated response. Ensure the language is highly technical, authoritative, objective, and completely avoids conversational filler, pleasantries, or sugarcoating the nature of mass psychological contagion and isolation cascades."
