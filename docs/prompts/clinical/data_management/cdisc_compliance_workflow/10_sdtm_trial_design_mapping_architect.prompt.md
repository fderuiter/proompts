---
title: SDTM Trial Design Mapping Architect
---

# SDTM Trial Design Mapping Architect
...
[View Source YAML](https://github.com/fderuiter/proompts/blob/main/prompts/clinical/data_management/cdisc_compliance_workflow/10_sdtm_trial_design_mapping_architect.prompt.yaml)

```yaml
---
name: SDTM Trial Design Mapping Architect
version: 1.0.0
description: Automates the complex algorithmic mapping and generation of CDISC SDTM Trial Design domains (TA, TE, TV) from clinical trial protocols and EDC schedule of assessments.
authors:
  - CDISC Architect
metadata:
  domain: clinical
...
variables:
  - name: protocol_schedule
    description: The Schedule of Assessments or study epoch definitions from the clinical protocol.
    required: true
  - name: edc_metadata
    description: EDC metadata containing visit schedules and arm allocations.
    required: true
  - name: sdtm_ig_version
    description: The target SDTM Implementation Guide version (e.g., 3.3, 3.4).
    required: true
...
testData:
  - input:
      sdtm_ig_version: "3.3"
      protocol_schedule: |
        Epochs: Screening, Double-Blind Treatment, Follow-up.
        Arms: Placebo, Active Drug.
        Treatment Epoch branches into either Placebo or Active.
      edc_metadata: |
        Visits: V1 (Day -14), V2 (Day 1), V3 (Day 14).
    expected: |
      {
        "TE_Domain": [
...
evaluators:
  ...
  - name: Output is JSON
    regex:
      pattern: (?s)^\{.*\}$
  - name: Includes TE Domain
    regex:
      pattern: (?s)"TE_Domain"
  - name: Includes TA Domain
    regex:
      pattern: (?s)"TA_Domain"
