---
title: CDISC Cross-Dataset Relational Architect
---

# CDISC Cross-Dataset Relational Architect
...
[View Source YAML](https://github.com/fderuiter/proompts/blob/main/prompts/clinical/data_management/cdisc_compliance_workflow/16_cdisc_cross_dataset_relrec_suppqual_architect.prompt.yaml)

```yaml
---
name: CDISC Cross-Dataset Relational Architect
version: 1.0.0
description: Automates the complex algorithmic mapping and harmonization of relational data across CDISC SDTM domains, specifically generating RELREC (Related Records) and integrating SUPPQUAL (Supplemental Qualifiers) correctly without structural violations.
authors:
  - CDISC Architect
metadata:
  domain: clinical
...
variables:
  - name: primary_domain_data
    description: Sample or structure of the primary SDTM domain data (e.g., AE, CM, EX).
    required: true
  - name: secondary_domain_data
    description: Sample or structure of the secondary or related SDTM domain data to be linked.
    required: true
...
testData:
  - input:
      primary_domain_data: |
        USUBJID, AESEQ, AETERM, AESTDTC, AEENDTC
        SUBJ-001, 1, HEADACHE, 2023-01-10, 2023-01-12
        SUBJ-001, 2, NAUSEA, 2023-01-15, 2023-01-16
      secondary_domain_data: |
        USUBJID, CMSEQ, CMTRT, CMSTDTC, CMENDTC, RELATED_AE
...
evaluators:
  ...
  - name: Output contains required keys
    regex:
      pattern: (?s)(RelationalAnalysis|RELRECMapping|SUPPQUALMapping|DataIntegrityChecks)
