...
testData:
  - input:
    ...
    expected: "Reportable"
  - input:
      complaint_description: "Patient 001-A failed screening during initial baseline tests."
      device_information: "Model X200 wearable."
      patient_impact: "None."
  ...
  - input:
      ...
      device_information: ""
      patient_impact: ""
    expected: "Error: Insufficient data to determine reportability."
evaluators:
  - name: FDA Reportability Check
    regex:
      pattern: "(?i)(Reportable|Not Reportable) under FDA 21 CFR 803"
  - name: EU MDR Reportability Check
    regex:
      pattern: "(?i)(Reportable|Not Reportable) under EU MDR 2017/745"
  - name: Timeline Check
    regex:
      pattern: "(?i)(5|15|30) days"
  - name: Edge Case Handled
    regex:
      pattern: "(?i)(insufficient data|need more information|error)"
