---
title: Multiple Endpoints Regulatory Strategy
---
...
```yaml
---
name: Multiple Endpoints Regulatory Strategy
version: 0.1.0
description: Review and summarize FDA guidance on multiple endpoints and multiplicity strategies.
metadata:
  domain: regulatory
  complexity: medium
  tags:
...
variables:
- name: issues
  description: The issues to use for this prompt
  required: true
- name: therapeutic_area
  description: The therapeutic area to use for this prompt
  required: true
model: gpt-4o
modelParameters:
  temperature: 0.1
messages:
- role: system
  content: 'You are a Regulatory Affairs Strategist and Biostatistician. Your task is to review the FDA Guidance for Industry
    ...
    3.  **## Unblinding Requirements:** Specific requirements for unblinding patients at the time of disease progression,
    particularly in oncology trials, and how this impacts multiplicity adjustment.
...
- input: 'therapeutic_area: Oncology
    ...
    '
  expected: Summary advising on splitting alpha between PFS/OS or using hierarchical testing, and handling potential unblinding
    after progression.
evaluators:
- name: Grouping/Ordering Check
  regex:
    pattern: (?i)(grouping|ordering|hierarchy|gatekeeping)
- name: Unblinding Check
  regex:
    pattern: (?i)(unblind|progression)
