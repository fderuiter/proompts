...
# Training Impact Analytics Planner
...
```yaml
---
name: Training Impact Analytics Planner
version: 0.1.0
description: Correlate training data with audit deviations and design interventions for high-risk learners.
metadata:
  domain: management
  complexity: medium
  tags:
...
variables:
- name: analysis_goal
  description: specific compliance or performance metric to improve
  required: true
model: gpt-4o
modelParameters:
  temperature: 0.2
messages:
- role: system
  content: 'You are acting as a learning data scientist for a mid-size global CRO. Available data includes:
...
- role: user
  content: '- `{{analysis_goal}}` – specific compliance or performance metric to improve.
