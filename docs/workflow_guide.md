...
```bash
python3 engine/scripts/run_workflow.py workflows/technical/software_engineering/paw_workflow.workflow.yaml -i product_concept="A new app idea"
```
...
```bash
python3 engine/scripts/run_workflow.py workflows/communication/joke_generator.workflow.yaml
```
...
```bash
python3 engine/scripts/run_workflow.py path/to/workflow.workflow.yaml [options]
```
...
**Fix:** Use a YAML linter or `python3 engine/scripts/validate_prompt_schema.py` to check your files.
