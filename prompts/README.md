...
Prompts should use one `domain:` tag (multiple are allowed but discouraged). If multiple are present, discovery tooling uses the first matching `domain:` tag.
...
- **Simulation**: You can simulate workflows using the `promptops workflow` script to test logic without API costs.
...
- **[`promptops validate`)**: Validates that all prompt files follow the required schema and naming conventions.
...
- **`promptops workflow`**: The engine that executes workflows and prompts.
...
5.  **Verify**: Run `promptops validate` to ensure your prompt is valid.
