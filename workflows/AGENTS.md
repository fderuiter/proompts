...
- **Test Your Workflow**: After creating or modifying a workflow, you must test it to ensure it runs correctly. Use the `promptops workflow` script.
...
```bash
promptops workflow workflows/your_workflow_name.workflow.yaml -i input_name="test_value"
```
...
- When asked to run a workflow, use the `promptops workflow` script.
